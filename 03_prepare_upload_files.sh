...
popd
REST="attestation Containerfile-base Containerfile-build Containerfile-reproduce evren_securebootca_cert.der "
...
REST+="do_review.sh evren_secureboot_signer_cert.der signer1_trust_chain.pem attestation2 "
for TO_CP in $REST; do
    /usr/bin/cp -f ${TO_CP} toupload/
done
NEW=`cat shim-img-v${SHIM_VERSION}.tar.bz2.shasum256`
rm -f shim-img-v${SHIM_VERSION}.tar.bz2.shasum256
